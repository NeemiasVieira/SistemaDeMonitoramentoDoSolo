...
const TodosOsRegistros = () => {
  ...
  const [dataDeInicioBusca, setDataDeInicioBusca] = useState<string>(null);
  const [dataDeFimBusca, setDataDeFimBusca] = useState<string>(null);

  //Hooks
  const { getAllRecordsPaginated, allRecordsPaginated, allRecordsPaginatedIsLoading } = useGetAllRecordsPaginated({idPlanta, pagina, registrosPorPag, dataDeInicioBusca: FormatarDatas.inputDateToIso(dataDeInicioBusca), dataDeFimBusca: FormatarDatas.inputDateToIso(dataDeFimBusca)});
