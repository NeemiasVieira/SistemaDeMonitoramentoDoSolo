...
export const CreateEditEspecie: React.FC<CreateEditEspecieProps> = ({
  action,
  setAction,
  especie,
}) => {
  ...
  const { confirmCreateSpecie, createSpecieIsLoading, newSpecie } =
    useCreateSpecie(especieI);
  const { confirmUpdateSpecie, updateSpecieIsLoading, specie } =
    useUpdateSpecie(especieI);
  ...
  useEffect(() => {
    if (especie) {
      const specieFromEspecie: Specie = {
        ...
        parametros: {
          nitrogenio: {
            min: especie?.parametros?.nitrogenio?.min,
            max: especie?.parametros?.nitrogenio?.max,
          },
          fosforo: {
            min: especie?.parametros?.fosforo?.min,
            max: especie?.parametros?.fosforo?.max,
          },
          potassio: {
            min: especie?.parametros?.potassio?.min,
            max: especie?.parametros?.potassio?.max,
          },
          luz: {
            min: especie?.parametros?.luz?.min,
            max: especie?.parametros?.luz?.max,
          },
          umidade: {
            min: especie?.parametros?.umidade?.min,
            max: especie?.parametros?.umidade?.max,
          },
          temperatura: {
            min: especie?.parametros?.temperatura?.min,
            max: especie?.parametros?.temperatura?.max,
          },
          pH: {
            min: especie?.parametros?.pH?.min,
            max: especie?.parametros?.pH?.max,
          },
        },
      };
  ...
  const handleChangeMin = (
    parametro: keyof Specie["parametros"],
        ...
        {Object.entries(especieI.parametros).map(([parametro, valor]) => (
          <div key={parametro} className="DivInputParametro">
            ...
            <input
              ...
              value={valor.min}
              onChange={(event) =>
                handleChangeMin(parametro as keyof Specie["parametros"], event)
              }
            />
            ...
            <input
              ...
              value={valor.max}
              onChange={(event) =>
                handleChangeMax(parametro as keyof Specie["parametros"], event)
              }
            />
          </div>
        ))}
